...
/**
 * A fourth kind of debt.
 *
 * Writes nothing — and this one genuinely could not, whatever anybody
 * wanted. `LoanKind` is stored as a string and nobody in an existing save
 * has ever been charged for a course, so there is no old student loan to
 * convert and no field to back-fill. People schooled from here take them.
 */
const V49_TO_V50: Migration = {
  from: 49,
  to: 50,
  describe: 'student loans; no existing save has one to convert',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 50, checksum: checksumOf(world) },
      world,
    }
  },
}
 ...
 *
 * Writes nothing. An absent `funding` means nobody was ever asked, which
 * is the truth for every course in every existing save — and the one
 * value that would be actively wrong to write is 'self', because a
 * veteran in an old save who went to college DID have a claim on the GI
 * Bill; the simulation simply never modelled it. Recording that they paid
 * their own way would be inventing a hardship.
 */
const V50_TO_V51: Migration = {
  from: 50,
  to: 51,
  describe: 'funding sources; an absent one means nobody was asked',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 51, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * Graduate school, RE codes, and a refusal that lasts.
 *
 * Writes nothing, and every absent field reads correctly without it:
 *
 *   `graduate` is APPENDED to the education ladder, so no stored level
 *   changes meaning — the same property that made inserting middle school
 *   safe, for the same reason (levels are stored as names, not ranks).
 *
 *   An absent `reCode` on a closed service record is read as CLEAN rather
 *   than barred. Refusing somebody at the recruiting desk on the strength
 *   of a field their save never had would be inventing a black mark.
 *
 *   An absent `declinedAtTick` means nothing has been turned down yet,
 *   which is exactly true of a save written before refusals were stamped.
 */
const V51_TO_V52: Migration = {
  from: 51,
  to: 52,
  describe: 'graduate level, RE codes, declined-question stamps',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 52, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * Shares in a named company.
 *
 * Writes nothing, and this is the one place the spec's suggested
 * migration was actively worth NOT doing. §7 proposes converting existing
 * sector holdings into "sector-fund instruments" — but they already ARE
 * that. A holding with no company named is exactly a fund position and
 * always was; the new field is optional and absent on every stored one.
 * Rewriting them all to mean what they already meant would be risk taken
 * for no change in behaviour.
 */
const V53_TO_V54: Migration = {
  from: 53,
  to: 54,
  describe: 'company holdings; an unnamed holding is a fund position, as before',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 54, checksum: checksumOf(world) },
      world,
    }
  },
}
 ...
 * Company news.
 *
 * Writes nothing and stores nothing new: an item is rolled, folded into
 * that month's price move, and recorded as an event. There is no news
 * STATE to migrate — what happened is in the ledger, and the consequence
 * is already in the price.
 */
const V54_TO_V55: Migration = {
  from: 54,
  to: 55,
  describe: 'company news; recorded as events, no new state',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 55, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * Hiring at the bottom, and a performance ceiling that includes the job.
 *
 * Writes nothing. Nobody loses a job — the change is about who can be
 * hired into WHAT from here, and about how good somebody can get at the
 * work they already do. An employee sitting on a senior rung in an
 * existing save keeps it; the rule that would no longer have put them
 * there does not reach backwards and take it away.
 */
const V55_TO_V56: Migration = {
  from: 55,
  to: 56,
  describe: 'entry-rung hiring and a caused performance ceiling; existing jobs keep their rung',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 56, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * A government: seats, ballots and the levers.
 *
 * Writes nothing, and the LOADER does the work — `save.ts` fills absent
 * government state with sane defaults on the way in. What an old save
 * needs is not a conversion but a STARTING POINT: no officials means the
 * first tick opens an election for every seat, no elections means no
 * campaign is half-run, and default policy is what a town that has never
 * argued about it would have.
 *
 * Nothing is invented. A world that had no government until now acquires
 * one the month it is loaded, which is honest — those seats did not exist
 * in that world before.
 */
const V56_TO_V57: Migration = {
  from: 56,
  to: 57,
  describe: 'a government; absent seats hold their first election on load',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 57, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * The first policy lever, wired.
 *
 * Writes nothing, and nobody's bill moves on load: `freshPolicy` starts
 * the property-tax rate at the 11 per mille that real estate charged as a
 * constant, so a world that has never had a government pays exactly what
 * it paid before. What changes is that a government can now move it.
 */
const V57_TO_V58: Migration = {
  from: 57,
  to: 58,
  describe: 'property tax reads the policy lever; the starting rate is unchanged',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 58, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * Police and school funding wired, halls, and the school choice.
 *
 * Writes nothing. Absent `inHalls` reads as "not in halls", which is true
 * of every student in every existing save — they were living wherever
 * they were living, and the field only decides what happens from here.
 */
const V58_TO_V59: Migration = {
  from: 58,
  to: 59,
  describe: 'halls, the school choice, and two more policy levers',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 59, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * The federal rate reads the lever. Phase 2 is complete.
 *
 * Writes nothing, and nobody's bill moves: the rate defaults to the 220
 * per mille the middle bracket was already written at, so an existing
 * save pays exactly what it paid. Only a government moving the number
 * changes anything.
 */
const V59_TO_V60: Migration = {
  from: 59,
  to: 60,
  describe: 'federal income tax reads the policy lever; the schedule is unchanged',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 60, checksum: checksumOf(world) },
      world,
    }
  },
}
 ...
 * Running for office.
 *
 * Writes nothing. An election in an existing save has no war chest and
 * no player among its runners, which is exactly true — nobody had stood
 * for anything before this. Absent reads as absent.
 */
const V60_TO_V61: Migration = {
  from: 60,
  to: 61,
  describe: 'candidacies, campaigns and the debate',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 61, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * Governing: the levers an officeholder pulls, and approval answering.
 *
 * Writes nothing. Every officeholder in an existing save already carries
 * an approval figure — it was seeded at 500 when the seat was filled —
 * and the levers were already stored. What is new is who may move them
 * and what it costs, neither of which is state.
 */
const V61_TO_V62: Migration = {
  from: 61,
  to: 62,
  describe: 'governing: lever permissions, approval, and the town budget',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 62, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * The state and national ladder, and the commander-in-chief.
 *
 * Writes nothing. An absent `militaryBudget` reads as the middle of its
 * range wherever it is asked, which is what a country that has never had
 * anybody in charge of the question was already doing. The upper seats
 * simply hold their first elections on the next tick.
 */
const V62_TO_V63: Migration = {
  from: 62,
  to: 63,
  describe: 'the ladder above the town, and war powers',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 63, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * Corruption, and a way to settle a debt.
 *
 * Writes nothing. An absent `exposure` reads as zero, which is the truth
 * about every officeholder in every existing save — nobody had been
 * offered anything yet. Debts are unchanged; what is new is a verb that
 * pays one off, which is not state.
 */
const V63_TO_V64: Migration = {
  from: 63,
  to: 64,
  describe: 'corruption and exposure; paying a debt down',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 64, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * CAREERS OVERHAUL, FIXES 3B AND 3C — the scale-up and the IPO.
 *
 * Three new pieces of state and every one of them is ABSENT-MEANS-NO on an
 * old save, which is why this migration moves no data:
 *
 *   `world.listings` — companies this town floated. An old world floated
 *     none, and the restore's `?? []` already reads that correctly.
 *   `business.scaledAtTick` — null on every business ever opened before
 *     today, because none of them had scaled up.
 *   `business.listedStockId` / `founderStakePerMille` — likewise.
 *
 * Nobody loses a business, a share or a rung. What changes is that the
 * ladder has one more rung on it and the market can now grow.
 */
const V64_TO_V65: Migration = {
  from: 64,
  to: 65,
  describe: 'company scale-up, the IPO, and a market that can be joined',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 65, checksum: checksumOf(world) },
      world,
    }
  },
}
 ...
 *
 * `world.gamblers` is new and every existing save has none: nobody in a
 * world written before today has been through the doors, so the restore's
 * `?? []` is the whole of it. No money moves, no account changes, and
 * nobody wakes up holding chips.
 */
const V65_TO_V66: Migration = {
  from: 65,
  to: 66,
  describe: 'the casino: chips, poker skill, and what gambling costs',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 66, checksum: checksumOf(world) },
      world,
    }
  },
}
 ...
 *
 * `world.athletes` is new and empty on every existing save — nobody in a
 * world written before today ever joined a team, which is also true of
 * most people in worlds written after. Nothing to move.
 */
const V66_TO_V67: Migration = {
  from: 66,
  to: 67,
  describe: 'athlete careers: the school-to-pro pipeline',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 67, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * M-HEALTH phase 1. Permanent conditions become structured.
 *
 * `HealthRecord.permanent` is new — the machine-readable twin of `marks`,
 * which was prose and so could never be reasoned about. Every existing
 * record gets an empty list.
 *
 * WHY EXISTING WOUNDS ARE NOT BACK-FILLED HERE. `marks` are English
 * sentences ("the loss of the left leg"); parsing them back into kind and
 * site would be guesswork, and a wrong guess writes a disability onto
 * somebody who does not have one. Saved lives keep their narrated marks and
 * their disability number; conditions recorded from here on carry effects.
 */
const V67_TO_V68: Migration = {
  from: 67,
  to: 68,
  describe: 'health: structured permanent conditions',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const health = Array.isArray(world['health']) ? world['health'] : []
    const migrated = health.map((entry) => {
      ...
      return 'permanent' in record ? record : { ...record, permanent: [] }
    })
    const nextWorld = { ...world, health: migrated }
    return {
      ...save,
      header: { ...header, schemaVersion: 68, checksum: checksumOf(nextWorld) },
      world: nextWorld,
    }
  },
}

const V68_TO_V69: Migration = {
  from: 68,
  to: 69,
  describe: 'health: the rating the Benefits Administration granted',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const health = Array.isArray(world['health']) ? world['health'] : []
    const migrated = health.map((entry) => {
      if (typeof entry !== 'object' || entry === null) return entry
      const record = entry as Record<string, unknown>
      if ('baRating' in record) return record
      /**
       * BACKFILL: a veteran already drawing the automatic pension has, in
       * every sense that matters, been granted their derived rating — the
       * board recognized it the day the threshold crossed. Writing that
       * number down changes nobody's pay by a cent; it only means the
       * rating letter they file for later starts from the truth. Everyone
       * else starts unrated.
       */
      const connected = typeof record['serviceDisability'] === 'number' ? record['serviceDisability'] : 0
      return { ...record, baRating: connected >= 200 ? connected : null }
    })
    const nextWorld = { ...world, health: migrated }
    return {
      ...save,
      header: { ...header, schemaVersion: 69, checksum: checksumOf(nextWorld) },
      world: nextWorld,
    }
  },
}
...
/**
 * THE SHADOW LEDGER, SWEPT UP.
 *
 * v70 merged every married couple's money onto the lower-id spouse. But the
 * engine went on WRITING wages, dividends, graft and sale proceeds to each
 * person's own record, so from the moment a v70 save resumed play a second
 * balance began growing again on the non-holder's file — money the Money tab
 * never showed and which loan payoffs quietly spent (two live reports: a
 * mortgage cleared "taking nothing from my actual money", and unlimited
 * casino chips). The engine now routes every liquid movement through the
 * wallet; this sweeps up what the old code stranded.
 *
 * Cent-conserving by construction: two adds, two zeroes, per couple. Only
 * checking and savings move — brokerage, retirement, loans, the deed, the
 * tax year and pension months are personal and stay exactly where they are.
 */
const V70_TO_V71: Migration = {
  from: 70,
  to: 71,
  describe: 'H0: sweep stranded shadow balances into the joint wallet',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const relationships = Array.isArray(world['relationships']) ? world['relationships'] : []
    ...
    for (const entry of relationships) {
      ...
      const b = typeof record['b'] === 'number' ? record['b'] : null
      if (a === null || b === null) continue
      const holder = accountByPerson.get(Math.min(a, b))
      const other = accountByPerson.get(Math.max(a, b))
      if (holder === undefined || other === undefined) continue
      if (num(other['checking']) === 0 && num(other['savings']) === 0) continue
      holder['checking'] = num(holder['checking']) + num(other['checking'])
      holder['savings'] = num(holder['savings']) + num(other['savings'])
      other['checking'] = 0
      other['savings'] = 0
    ...
    return {
      ...save,
      header: { ...header, schemaVersion: 71, checksum: checksumOf(nextWorld) },
      world: nextWorld,
    }
  },
}
...
/**
 * THE SHARE REGISTER ARRIVES.
 *
 * A business can now have people and firms owning pieces of it. Nothing to
 * convert: a save written before this had no registers, and a business with
 * no register is one wholly its founder's — which is exactly what every
 * business in an old save was. The empty map is the honest migration.
 */
const V71_TO_V72: Migration = {
  from: 71,
  to: 72,
  describe: 'businesses can have shareholders',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const nextWorld = { ...world, capTables: Array.isArray(world['capTables']) ? world['capTables'] : [] }
    return {
      ...save,
      header: { ...header, schemaVersion: 72, checksum: checksumOf(nextWorld) },
      world: nextWorld,
    }
  },
}
...
/**
 * BUSINESSES CAN GROW BEYOND THEIR OWN DOORS.
 *
 * Nothing to convert: a save written before this had no expansions, and a
 * business with none is one that never grew — which every business in an
 * older save is. The empty list is the honest migration.
 */
const V72_TO_V73: Migration = {
  from: 72,
  to: 73,
  describe: 'businesses can open second places, franchise and buy suppliers',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const nextWorld = {
      ...world,
      expansions: Array.isArray(world['expansions']) ? world['expansions'] : [],
    }
    return {
      ...save,
      header: { ...header, schemaVersion: 73, checksum: checksumOf(nextWorld) },
      world: nextWorld,
    }
  },
}
...
/**
 * A BUSINESS KEEPS ITS BOOKS NOW.
 *
 * Nothing to convert: a save written before this has no months recorded,
 * and an empty set of books is exactly what a business whose history was
 * never kept has. It fills from the next month traded.
 */
const V73_TO_V74: Migration = {
  from: 73,
  to: 74,
  describe: 'businesses keep two years of books',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const nextWorld = {
      ...world,
      businessBooks: Array.isArray(world['businessBooks']) ? world['businessBooks'] : [],
    }
    return {
      ...save,
      header: { ...header, schemaVersion: 74, checksum: checksumOf(nextWorld) },
      world: nextWorld,
    }
  },
}
...
/**
 * A BUSINESS IS RUN NOW, not merely owned.
 *
 * Nothing to convert: a save written before this has no stockroom, no
 * supplier and no price list, which is exactly what a business nobody was
 * actively running looks like. The record is made the first time its owner
 * touches any of it.
 */
const V74_TO_V75: Migration = {
  from: 74,
  to: 75,
  describe: 'businesses carry stock, a supplier and a price',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const nextWorld = {
      ...world,
      businessOps: Array.isArray(world['businessOps']) ? world['businessOps'] : [],
    }
    return {
      ...save,
      header: { ...header, schemaVersion: 75, checksum: checksumOf(nextWorld) },
      world: nextWorld,
    }
  },
}
...
/**
 * A DEED REMEMBERS WHAT IT COST (owner, playing: "when you click on details
 * on the property it should show things like how much the value has changed
 * over time... like bought price stuff like that").
 *
 * Nothing to rewrite: the two fields are optional and absent means "not
 * recorded", which is the truth for every house bought before this and for
 * anything inherited. The screen says so rather than printing a gain it
 * cannot know.
 */
const V75_TO_V76: Migration = {
  from: 75,
  to: 76,
  describe: 'deeds remember what they cost and when',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    return {
      ...save,
      header: { ...header, schemaVersion: 76, checksum: checksumOf(world) },
      world,
    }
  },
}
...
/**
 * SCHEMA 77 — EVERY PERSON CAN NOW BE GOOD AT SOMETHING (jobs revamp).
 *
 * Skill sheets are what WORK left behind, and an existing save has no record
 * of which months were spent doing what — so nobody is awarded a skill they
 * cannot be shown to have earned. The map opens empty and fills from the
 * work done from here on, which is the only honest reading of a save that
 * predates the system.
 *
 * The loader already defaults a missing `skills` to an empty map, so this
 * step is a version bump and a re-checksum rather than a data rewrite. It
 * exists so the chain has no hole in it: a version without a migration is
 * the bug the runner throws on.
 */
const V76_TO_V77: Migration = {
  from: 76,
  to: 77,
  describe: 'skills and licences: what the work left behind, and the papers held',
  apply(save) {
    const header = requireObject(requireField(save, 'header', 'save'), 'save.header')
    const world = requireObject(requireField(save, 'world', 'save'), 'save.world')
    const next = {
      ...world,
      skills: Array.isArray(world['skills']) ? world['skills'] : [],
      licences: Array.isArray(world['licences']) ? world['licences'] : [],
    }
    return {
      ...save,
      header: { ...header, schemaVersion: 77, checksum: checksumOf(next) },
      world: next,
    }
  },
}

const MIGRATIONS: readonly Migration[] = [V1_TO_V2, V2_TO_V3, V3_TO_V4, V4_TO_V5, V5_TO_V6, V6_TO_V7, V7_TO_V8, V8_TO_V9, V9_TO_V10, V10_TO_V11, V11_TO_V12, V12_TO_V13, V13_TO_V14, V14_TO_V15, V15_TO_V16, V16_TO_V17, V17_TO_V18, V18_TO_V19, V19_TO_V20, V20_TO_V21, V21_TO_V22, V22_TO_V23, V23_TO_V24, V24_TO_V25, V25_TO_V26, V26_TO_V27, V27_TO_V28, V28_TO_V29, V29_TO_V30, V30_TO_V31, V31_TO_V32, V32_TO_V33, V33_TO_V34, V34_TO_V35, V35_TO_V36, V36_TO_V37, V37_TO_V38, V38_TO_V39, V39_TO_V40, V40_TO_V41, V41_TO_V42, V42_TO_V43, V43_TO_V44, V44_TO_V45, V45_TO_V46, V46_TO_V47, V47_TO_V48, V48_TO_V49, V49_TO_V50, V50_TO_V51, V51_TO_V52, V52_TO_V53, V53_TO_V54, V54_TO_V55, V55_TO_V56, V56_TO_V57, V57_TO_V58, V58_TO_V59, V59_TO_V60, V60_TO_V61, V61_TO_V62, V62_TO_V63, V63_TO_V64, V64_TO_V65, V65_TO_V66, V66_TO_V67, V67_TO_V68, V68_TO_V69, V69_TO_V70, V70_TO_V71, V71_TO_V72, V72_TO_V73, V73_TO_V74, V74_TO_V75, V75_TO_V76, V76_TO_V77]
