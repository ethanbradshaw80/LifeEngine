 ...
 *      candidate states a preference the branch may or may not honour.
 *      Every service record carries the score it was written with and the
 *      track it entered on, so an unplayed world's bytes move even where
 *      nobody's history did.
 ...
 */
export const SIMULATION_VERSION = 189
...
export function toSnapshot(world: World): WorldSnapshot {
  return {
    ...
    body: {
      ...
      stockPrices: world.stockPrices,
      stockHistory: world.stockHistory,
      officials: [...world.officials.values()].sort((a, b) =>
        a.officeId < b.officeId ? -1 : a.officeId > b.officeId ? 1 : 0,
      ),
      elections: [...world.elections.values()].sort((a, b) =>
        a.officeId < b.officeId ? -1 : a.officeId > b.officeId ? 1 : 0,
      ),
      policy: world.policy,
      gamblers: [...world.gamblers.values()].sort((a, b) => a.personId - b.personId),
      athletes: [...world.athletes.values()].sort((a, b) => a.personId - b.personId),
      ...
      habits: [...world.habits.values()].sort((a, b) => a.personId - b.personId),
      properties: [...world.properties.values()].sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0)),
      leases: [...world.leases.values()].sort((a, b) => a.householdId - b.householdId),
      /**
       * SKILL SHEETS, sparse and sorted (jobs revamp). Only people who have
       * actually worked at something appear, which keeps a save from
       * carrying eighteen zeroes for every child in the town.
       */
      skills: [...world.skills.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([personId, sheet]) => ({ personId, sheet })),
      licences: [...world.licences.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([personId, held]) => ({ personId, held })),
      businessOps: [...world.businessOps.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([businessId, ops]) => ({ businessId, ...ops })),
      moneyLog: world.moneyLog,
      trusts: world.trusts,
      businessBooks: [...world.businessBooks.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([businessId, months]) => ({ businessId, months })),
      expansions: [...world.expansions.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([businessId, list]) => ({ businessId, list })),
      capTables: [...world.capTables.entries()]
        .sort((a, b) => a[0] - b[0])
        .map(([businessId, table]) => ({ businessId, ...table })),
